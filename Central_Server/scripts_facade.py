...
class ScriptsFacade:
    ...
    def load_script_meta(self):
        ...
        if 'services' not in state: state = {'services': [], 'teams': [], 'scripts': []}
    ...
    def publish_tasks(self, tick_id):
        ...
        test_task = {"team_id": 1, "service_id": 1, "script": "test_script.py", "ip": "127.0.0.1", "port": 80, "type": "benign"}
        channel.basic_publish(exchange="", routing_key="service_queue", body=json.dumps(test_task))
        connection.close()
        print("DEBUG: Da tu dong gui Task vao service_queue!")

        # Get meta-information on all the scripts
        scripts, services = [1], [1]
